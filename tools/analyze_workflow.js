...
function analyzeContext(raw) {
  ...
  const recs = [];
  if (!lower.includes('charter') && !lower.includes('requirements')) {
    recs.push({
      title: "Missing charters",
...
## Recommended Flow
1. **charter**: Write a \`charter.md\` first.
2. **Build**: Use ${tools[0] || 'AI'} to generate code against the charter.
3. **Verify**: Run tests tailored to the charter.
`
  };
}
