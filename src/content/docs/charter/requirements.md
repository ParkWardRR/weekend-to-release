...
For data-heavy features, the charter includes a Key Entities sub-section defining the data model at a conceptual level:
...
The agent adds these markers during `/speckit.charter`. You resolve them with `/speckit.clarify`. This prevents the common failure mode where the AI assumes an answer and builds the wrong thing.
