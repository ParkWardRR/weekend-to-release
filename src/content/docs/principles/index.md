...
description: Governance rules that keep SDD projects on track.
---

The **constitution** is the governance layer of Specification-Driven Development. It defines non-negotiable rules that every specification, plan, and task list must satisfy.
...
Articles act as **phase gates** — checkpoints that specifications must pass before moving to the next phase.

```mermaid
flowchart TD
    S[Specify] --> G1{Gate Check}
    G1 -->|Pass| P[Plan]
    G1 -->|Fail| S
    P --> G2{Gate Check}
    G2 -->|Pass| T[Tasks]
...
- A spec requiring 5 microservices for a simple CRUD app violates **Article VII (Simplicity)**
